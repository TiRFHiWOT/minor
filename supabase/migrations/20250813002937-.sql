-- Fix ambiguous column reference in get_enriched_topics function
DROP FUNCTION IF EXISTS get_enriched_topics(uuid, integer, integer, text);
...
BEGIN
  ...
  LEFT JOIN LATERAL (
    SELECT posts.id as post_id, posts.author_id
    FROM posts 
