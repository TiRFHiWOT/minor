-- Complete cleanup of functions referencing old table names
...
-- Remove any remaining triggers referencing old tables
DROP TRIGGER IF EXISTS sync_topic_to_forum ON topics;
DROP FUNCTION IF EXISTS sync_topic_to_forum();

-- Ensure all forum stat functions use correct table names
DROP FUNCTION IF EXISTS get_enriched_topics(uuid, integer, integer);
DROP FUNCTION IF EXISTS get_enriched_topics_count(uuid);