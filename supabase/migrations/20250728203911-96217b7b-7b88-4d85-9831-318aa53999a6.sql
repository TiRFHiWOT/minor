-- Create notification_type enum if it doesn't exist
CREATE TYPE notification_type AS ENUM ('new_post', 'new_report', 'content_pending', 'new_appeal');

-- Add support for admin moderation notifications
-- Add new fields to topic_notifications for reports and appeals
ALTER TABLE public.topic_notifications 
ADD COLUMN IF NOT EXISTS report_id uuid REFERENCES public.reports(id) ON DELETE CASCADE,
ADD COLUMN IF NOT EXISTS appeal_id uuid REFERENCES public.moderation_appeals(id) ON DELETE CASCADE;

-- Handle the notification_type column properly by updating existing data first
UPDATE public.topic_notifications SET notification_type = 'new_post' WHERE notification_type IS NULL OR notification_type = '';

-- Now alter the column to use the enum, dropping default first
ALTER TABLE public.topic_notifications ALTER COLUMN notification_type DROP DEFAULT;
ALTER TABLE public.topic_notifications 
ALTER COLUMN notification_type TYPE notification_type USING 
CASE 
  WHEN notification_type = 'new_post' THEN 'new_post'::notification_type
  ELSE 'new_post'::notification_type  
END;

-- Set the default back
ALTER TABLE public.topic_notifications ALTER COLUMN notification_type SET DEFAULT 'new_post'::notification_type;

-- Function to notify all admins
CREATE OR REPLACE FUNCTION notify_all_admins(
  p_notification_type notification_type,
  p_topic_id uuid DEFAULT NULL,
  p_post_id uuid DEFAULT NULL,
  p_report_id uuid DEFAULT NULL,
  p_appeal_id uuid DEFAULT NULL
) RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  -- Insert notifications for all admin users
  INSERT INTO public.topic_notifications (user_id, topic_id, post_id, report_id, appeal_id, notification_type)
  SELECT 
    ur.user_id,
    p_topic_id,
    p_post_id,
    p_report_id,
    p_appeal_id,
    p_notification_type
  FROM public.user_roles ur
  WHERE ur.role = 'admin';
END;
$$;

-- Trigger function for new reports
CREATE OR REPLACE FUNCTION notify_admins_new_report()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  PERFORM notify_all_admins(
    'new_report'::notification_type,
    NEW.reported_topic_id,
    NEW.reported_post_id,
    NEW.id,
    NULL
  );
  RETURN NEW;
END;
$$;

-- Trigger function for new appeals
CREATE OR REPLACE FUNCTION notify_admins_new_appeal()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  PERFORM notify_all_admins(
    'new_appeal'::notification_type,
    CASE WHEN NEW.content_type = 'topic' THEN NEW.content_id ELSE NULL END,
    CASE WHEN NEW.content_type = 'post' THEN NEW.content_id ELSE NULL END,
    NULL,
    NEW.id
  );
  RETURN NEW;
END;
$$;

-- Create triggers
DROP TRIGGER IF EXISTS trigger_notify_admins_new_report ON public.reports;
CREATE TRIGGER trigger_notify_admins_new_report
  AFTER INSERT ON public.reports
  FOR EACH ROW
  EXECUTE FUNCTION notify_admins_new_report();

DROP TRIGGER IF EXISTS trigger_notify_admins_new_appeal ON public.moderation_appeals;
CREATE TRIGGER trigger_notify_admins_new_appeal
  AFTER INSERT ON public.moderation_appeals
  FOR EACH ROW
  EXECUTE FUNCTION notify_admins_new_appeal();