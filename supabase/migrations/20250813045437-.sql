...
BEGIN
  ...
  SET 
    meta_title = CASE 
      WHEN topics.meta_title IS NULL OR topics.meta_title = '' THEN
        topics.title || ' | Minor Hockey Forum | ' || COALESCE(categories.name, 'General')
      ELSE topics.meta_title -- Keep existing custom titles
    END,
    meta_description = CASE 
      WHEN topics.meta_description IS NULL OR topics.meta_description = '' THEN
        CASE 
        ...
        END
      ELSE topics.meta_description -- Keep existing custom descriptions
    END,
    updated_at = now()
  FROM categories
...
BEGIN
  ...
  SET 
    meta_title = CASE 
      WHEN categories.meta_title IS NULL OR categories.meta_title = '' THEN
        categories.name || ' | Minor Hockey Forum'
      ELSE categories.meta_title -- Keep existing custom titles
    END,
    meta_description = CASE 
      WHEN categories.meta_description IS NULL OR categories.meta_description = '' THEN
        'Explore ' || categories.name || ' topics and join the discussion on Minor Hockey Forum.'
      ELSE categories.meta_description -- Keep existing custom descriptions
    END
