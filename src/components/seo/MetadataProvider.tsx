...
export const MetadataProvider: React.FC<MetadataProviderProps> = ({
  ...
  const autoGenerationEnabled = getSetting(
    "seo_auto_generate_topic_titles",
    true
  );
  ...
  const { data: categoryMetadata } = useQuery({
    ...
    queryFn: async () => {
      ...
      const categorySlug = params.subcategorySlug || params.categorySlug;
  ...
  const truncate = (s: string, len = 160) =>
    s.length > len ? s.slice(0, len - 1).trimEnd() + "…" : s;

  // Determine page metadata based on current route
  // Move TopicMetadataType interface to top-level scope
  interface TopicMetadataType {
    id: string;
    title?: string;
    slug?: string;
    meta_title?: string;
  ...
  const getPageMetadata = (): PageMetadata => {
    ...
    if (Object.keys(customMetadata).length > 0) {
      return {
        title: customMetadata.title
          ? `${customMetadata.title}${titleSeparator}${forumName}`
          : undefined,
        ...customMetadata,
      };
    }
    ...
    if (topicMetadata && params.topicSlug) {
      const catName =
        (topicMetadata as TopicMetadataType).category_name ||
        formatSlug(params.subcategorySlug || params.categorySlug || "");

      console.log(
        "🏷️ MetadataProvider: Generating topic title with components:",
        {
      ...
      if (topicMetadata.meta_title) {
        ...
        title = `${
          topicMetadata.title || formatSlug(params.topicSlug!)
        }${titleSeparator}${forumName}`;
    ...
    if (params.topicSlug && !isTopicLoading && !topicMetadata) {
      ...
      console.log(
        "⚠️ MetadataProvider: Using fallback topic title (query finished, no data):",
        fallbackTitle
      );
    ...
    if (params.topicSlug && isTopicLoading) {
      console.log(
        "⏳ MetadataProvider: Topic query loading, using minimal title"
      );
      return {
        title: forumName,
    ...
    if (
      ...
      console.log(
        "📝 MetadataProvider: Generating blog post metadata for:",
        params.slug
      );
    ...
    if (categoryMetadata && params.categorySlug && !params.topicSlug) {
      const categoryName =
        categoryMetadata.name || formatSlug(params.categorySlug);

      // Use database meta_title if it exists, otherwise auto-generate
      const title =
        categoryMetadata.meta_title ||
        generateCategoryTitle(categoryName, forumName, titleSeparator);
      const description =
        categoryMetadata.meta_description ||
        generateCategoryDescription(categoryName, forumName);
  ...
  console.log(
    ...
    categoryMetadata ? "LOADED" : "NULL"
  );
  console.log(
    "💾 Blog post metadata state:",
    blogPostMetadata ? "LOADED" : "NULL"
  );
  ...
  return (
    <MetadataContext.Provider value={{ setPageMetadata }}>
      ...
      {/* Render JSON-LD graph for SEO and rich results. Keep payload minimal and safe. */}
      <JsonLd
        siteUrl={
          (getSetting("site_url", "") ||
        ...
        breadcrumb={(() => {
          // Build a small breadcrumb trail for category/topic pages
          const crumbs: { name: string; url: string }[] = [];
          if (location.pathname === "/") return crumbs;
          // Home
          crumbs.push({ name: FORUM_NAME, url: "/" });
          if (params.categorySlug) {
            crumbs.push({
              name: formatSlug(params.categorySlug),
              url: `/${params.categorySlug}`,
            });
          }
          if (params.subcategorySlug) {
            crumbs.push({
              name: formatSlug(params.subcategorySlug),
              url: `/${params.categorySlug}/${params.subcategorySlug}`,
            });
          }
          if (params.topicSlug) {
            crumbs.push({
              name: formatSlug(params.topicSlug),
