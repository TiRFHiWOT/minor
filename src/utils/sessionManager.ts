...
class SessionManager {
  ...
  async checkRateLimit(): Promise<{
    ...
    // Call backend rate limit function (via Supabase RPC or REST API)
    // Example: supabase.rpc('check_anonymous_rate_limit', { user_ip: ip, session_id: sessionId })
    try {
      ...
      if (error || !data) {
        // Fallback: allow posting, but show 0 remaining
        return { canPost: true, remainingPosts: 0 };
      }
      // data should contain { allowed: boolean, limit: number, current_count: number }
      return {
        canPost: data.allowed,
        remainingPosts: Math.max(
          0,
          (data.limit ?? 5) - (data.current_count ?? 0)
        ),
      };
    } catch (err) {
      // Fallback: allow posting, but show 0 remaining
